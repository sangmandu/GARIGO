...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        # pop from trackers and save to new trackers
        # 1) to extract face tracked
        # 2) to rid of face not tracked
        tracked_image = img_raw.copy()
        temp_trackers = []
        while trackers:
            # print(len(trackers))
            name, tracker = trackers.pop()
            success, bbox = tracker.update(tracked_image)
            if success:
                x, y, w, h = bbox
                cv2.circle(tracked_image, (x + w // 2, y + h // 2), w, (0, 0, 0), -1)
                temp_trackers.append((name, tracker))
        trackers = temp_trackers[:]
        img = np.float32(tracked_image)
            ...
            for b in dets:
                ...
                for (top, right, bottom, left), name in zip(face_locations, face_names):
                    ...
                    if name != "Unknown":
                        ...
                        tracker.init(img_raw, bbox)
                        trackers.append((name, tracker))
                    else:
                        ...
                        img_raw = cv2.rectangle(img_raw, (left, top), (right, bottom), (255, 0, 0), 3)
