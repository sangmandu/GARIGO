...
def mosaic_video(recog_list, recog_images, video_route, video_name):
    ...
    for person_name, img_urls in zip(recog_list, recog_images):
        ...
        for i, url in enumerate(img_urls):
            ...
            face_locations = face_recognition.face_locations(face, model='cnn', number_of_times_to_upsample=2)
            ...
            if len(face_locations) == 1:
                try:
                    ...
                    print(person_name + "/" + str(
                        i) + " was skipped and can't be used for training cuz not detecting face")
            else:
                print(person_name + "/" + str(
                    i) + " was skipped and can't be used for training cuz not detecting only one face ")
    ...
    for thresh in [0.35]:
        for match in [0.8111]:
            ...
            # testing begin
            while cap.isOpened() and frame_number < 1:
                ...
                face_locations = face_recognition.face_locations(img_raw, model='cnn', number_of_times_to_upsample=2)
